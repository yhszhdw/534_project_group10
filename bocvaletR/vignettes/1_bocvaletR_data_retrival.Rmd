---
title: "1. bocvaletR_data_retrival"
author: "Zihao Sheng"
date: "2026-01-31"
...
::: callout-note
All Bank of Canada time series are uniquely identified by a **series ID**. These IDs can be obtained using `boc_list_series()` or extracted from the output of `boc_list_groups()` and `boc_groups()`. Once a valid series ID is available, the corresponding observations can be retrieved directly using the `boc_series()` function.
:::
...
::: callout-note
In addition to retrieving historical time series data, bocvaletR provides access to the Bank of Canada foreign exchange RSS feed. This feed is designed for monitoring **recent updates** to exchange rate series rather than for retrieving long historical records.
:::
